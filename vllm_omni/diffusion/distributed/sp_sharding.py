...
def sp_shard(
    ...
    # Ensure tensor is large enough to shard
    if size < world_size:
        raise ValueError(
            f"Tensor size along dim {dim} ({size}) must be >= world_size ({world_size}) "
            f"for sequence parallel sharding. Tensor shape: {tensor.shape}"
        )
